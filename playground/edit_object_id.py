...
if current_reid_mapping is not None:
    _rest_objects = 0
    for v in current_reid_mapping['reid'].keys():
...
objects_to_reid = st.selectbox('待标注对象ID', sorted(list(realsense_dataset.led_tracking_result['result'][view_to_process].keys()), key=lambda x: int(x)), index=min(len(list(realsense_dataset.led_tracking_result['result'][view_to_process].keys())) - 1,st.session_state['next_objects_to_reid']) )
...
object_frame_index_relative_to_sequence = [int(x['frame_idx']) for x in realsense_dataset.recordings[view_to_process].color.metadata].index(int(object_frame_meta['meta']['frame_idx']))#  - int(realsense_dataset.recordings[view_to_process].color.metadata[0]['frame_idx'])
object_frame = cv2.imread(osp.join(realsense_dataset.path_to_stream, view_to_process, "color", object_frame_meta['meta']['basename']))
object_reference_frame = cv2.imread(osp.join(realsense_dataset.path_to_stream, CAMERA_REFERENCE_MAP[view_to_process], "color", realsense_dataset.recordings[CAMERA_REFERENCE_MAP[view_to_process]].color.metadata[object_frame_index_relative_to_sequence]['basename']))
...
if st.button('保存'):
    ...
    st.session_state['next_objects_to_reid'] = min(len(list(realsense_dataset.led_tracking_result['result'][view_to_process].keys())) - 1, max(0, list(realsense_dataset.led_tracking_result['result'][view_to_process].keys()).index(objects_to_reid)) + 1)
    st._rerun()
else:
    pass
if current_reid_mapping is not None:
