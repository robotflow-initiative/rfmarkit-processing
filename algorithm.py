...
class IMUAlgorithm(object):
    ...
    @classmethod
    def quat_to_pose_mat_np(cls, quat_data: np.ndarray) -> np.ndarray:
        """Convert quat data to transform matrix

        Args:
            quat_data (np.ndarray): 2-D matrix
            [[w0, x0, y0, z0],[w1, x1, y1, z1],...]

        Returns:
            np.ndarray: [description]
        """
        length = quat_data.shape[0]
        pose_mat = np.empty(shape=(length, 3, 3))
        for idx in range(length):
            pose_mat[idx] = t3d.quaternions.quat2mat(quat_data[idx])
        return pose_mat

    @classmethod
    def pose_mat_to_rpy_np(cls, mat_data: np.ndarray) -> np.ndarray:
        """Convert transform matrix to roll-pitch-yaw data

        Args:
            posemat (np.ndarray): 3-D matrix
            [[3x3],[3x3],...]
        ...
        rpy_mat = np.empty(shape=(length, 3))
        for idx in range(length):
            rpy_mat[idx] = t3d.euler.mat2euler(mat_data[idx])
        return rpy_mat
    

    @classmethod
    def pose_mat_to_quat_np(cls, mat_data: np.ndarray) -> np.ndarray:
        """Convert transform matrix to quaterions
        ...
        """
        length = mat_data.shape[0]
        quat_mat = np.empty(shape=(length, 4))
        for idx in range(length):
            quat_mat[idx] = t3d.quaternions.mat2quat(mat_data[idx])
        return quat_mat
