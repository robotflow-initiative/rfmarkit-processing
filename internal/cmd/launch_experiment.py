...
import cv2
import yaml
from articulated_kit.client import RealsenseRecorderClient, TcpBrokerClient
from articulated_kit.model import DefaultExperiment
...
class ExperimentConsole(cmd.Cmd):
    ...
    def _flush_experiment(self):
        self.index_yaml_dict['articulated_kit'] = self.experiment.get_dict()
        with open(self.experiment_index_filename, 'w') as f:
            yaml.dump(self.index_yaml_dict, f)
        pass

    def do_camera_calibrate(self, arg):
        from realsense_recorder.cmd.local_capture_static_interactive import main as entry_point
        from articulated_kit.client import run_multical_with_docker
        _arg = argparse.Namespace()
        _arg.config = self.realsense_config_filename
        collision, collision_idx = self._get_existing_calibrations()
        if len(collision) > 0:
            idx = max(collision_idx) + 1
        else:
            idx = 1
        self.console.log(f"saving as calibration index {idx}")
        _arg.tag = f"_cali-{idx}"
        try:
            entry_point(_arg)
            _, err = run_multical_with_docker(osp.join(self.experiment.base_dir, _arg.tag))
            if err is not None:
                self.console.log("Error during calibration, removing calibration result")
                shutil.rmtree(osp.join(self.experiment.base_dir, _arg.tag), ignore_errors=True)
        except Exception as _:
            self.console.log("Error during capture, removing calibration result")
            shutil.rmtree(osp.join(self.experiment.base_dir, _arg.tag), ignore_errors=True)
    ...
    def do_new(self, arg):
        ...
        from articulated_kit.client.realsense_recorder.api.default import status_v1_status_get as realsense_recorder_status
        from articulated_kit.client.realsense_recorder.api.default import stop_process_v1_stop_post as realsense_recorder_stop
        from articulated_kit.client.realsense_recorder.api.default import start_process_v1_start_post as realsense_recorder_start
        from articulated_kit.client.realsense_recorder.api.default import ready_v1_ready_get as realsense_recorder_ready

        from articulated_kit.client.tcpbroker.api.default import status_v1_status_get as tcpbroker_status
        from articulated_kit.client.tcpbroker.api.default import stop_process_v1_stop_post as tcpbroker_stop
        from articulated_kit.client.tcpbroker.api.default import start_process_v1_start_post as tcpbroker_start
        from articulated_kit.client.tcpbroker.api.default import imu_control_v1_imu_control_post as tcpbroker_control
        from articulated_kit.client.tcpbroker.api.default import imu_connection_v1_imu_connection_get as tcpbroker_connection
